...
var CustomRoomCard = class extends HTMLElement {
  ...
  _chips(room) {
    return Object.entries(room.entities || {}).flatMap(([category, selected]) => {
      ...
      return (Array.isArray(selected) ? selected : selected ? [selected] : []).map((item) => typeof item === "string" ? { entity: item } : item).filter((item) => item?.entity && domains.includes(item.entity.split(".")[0]) && this._hass.states[item.entity]);
    });
  }
  _render() {
...
var CustomRoomCardEditor = class extends HTMLElement {
  ...
  _roomEditor(parent, room, index) {
    ...
    Object.entries(CATEGORIES).forEach(([key, meta]) => {
      ...
      const selected = (Array.isArray(room.entities?.[key]) ? room.entities[key] : []).map((item) => typeof item === "string" ? { entity: item } : item).filter((item) => item?.entity);
