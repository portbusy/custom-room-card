...
class CustomRoomCardEditor extends HTMLElement {
  ...
  _selectedEntityRow(holder, chip, domains, roomIndex, category, entityIndex) {
    ...
    const renderChipContent = () => {
      ...
      const name = document.createElement("ha-selector"); name.className = "full-width-field"; name.hass = this._hass; name.label = "Etichetta"; name.selector = { template: {} }; name.value = chip.name || ""; this._handlePicker(name, (value) => this._updateChip(roomIndex, category, entityIndex, { name: value || undefined }));
