...
class CustomRoomCard extends HTMLElement {
  ...
  _room(room, ids, roomIndex) {
    ...
    return `<section class="room" style="--room-color:${escape(color)}"><button class="header" data-entity="${escape(room.motion_entity || ids[0] || "")}" aria-label="${escape(title)}"><span class="title"><ha-icon class="room-title-icon" icon="${escape(room.icon || area?.icon || "mdi:home")}"></ha-icon>${escape(title)}${lastMotion ? `<span class="motion-time">${lastMotion}</span>` : ""}</span><span class="summary">${status}${metrics}</span></button>${chips.length ? `<div class="chips">${chips.map((chip) => this._chip(chip)).join("")}</div>` : `<div class="empty">Nessuna entità selezionata</div>`}</section>`;
  }
  _chip(chip) {
...
class CustomRoomCardEditor extends HTMLElement {
  ...
  _render() {
    ...
    if (!this.shadowRoot.querySelector(".editor")) {
      this.shadowRoot.innerHTML = `<style>${EDITOR_STYLE}</style><div class="editor"><div class="controls"><ha-switch id="sort" ${this._config.sort_by_motion ? "checked" : ""}></ha-switch><label for="sort">Ordina le stanze per movimento</label><ha-switch id="entity-icons" ${this._config.show_entity_icons ? "checked" : ""}></ha-switch><label for="entity-icons">Usa icone reali delle entità</label></div><div class="category-order-section"><h5>Ordinamento categorie</h5><div id="category-order-list"></div></div><div id="rooms"></div><ha-button id="add">Aggiungi stanza</ha-button></div>`;
    ...
    // Rimuove l'altezza minima nel frame successivo
